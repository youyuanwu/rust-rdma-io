...
pub async fn run_read_ring_busy_client(
    ...
    // ready, and blocks until *all* are ready before the shared warmup/bench
    // clock and the CPU/RSS sampler start — so setup time (and CM retries) never
    // land in the measurement window (design §12 / §13). Each connection's echo
    // loop runs on its owning core (co-location, §7.5).
...
pub async fn run_read_ring_park_client(
    ...
    // + sampler only after every connection has finished setup, so setup time is
    // excluded from the window and busy-vs-park stays comparable (design §12 / §13).
    let outcome = run_readiness_barrier(connections, warmup, duration, |ready, gate| {
        let p = payload_buf.clone();
        let cfg = config.clone();
