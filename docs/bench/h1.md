...
- **`rh1-busy`** — lowest latency and highest throughput per core when the cores
  stay busy; costs 100 % CPU per pinned core even when idle. Best for
  latency-critical, steady-load HTTP/1.1.
- **`rh1-park`** — pinned locality without the idle spin (parks at ~0 % when
  quiet); ~1.6× the shared runtime at load, the shared runtime's low idle CPU.
  Best for bursty load.
