...
The first ceiling hunt hit a *client* file-descriptor limit, not a transport
limit: the distro default soft `ulimit -n` is **1024**, and each RDMA connection
uses ~3 fds (CM id + QP + event channel) while TCP uses ~1. That capped the rings
at ~384 connections (`384 × 3 > 1024`, `EMFILE`) and TCP at ~960 — artifacts, not
real peaks. The hard limit was already 1048576, so the soft limit is now raised
...
After the fix, `tcp1` scales to 5120+ connections and `read-ring` past 512 with 0
`EMFILE` errors, so the peaks in this doc are the true latency/stack/CM-setup
ceilings.
