...
apples-to-apples kernel baseline, and `--in-flight N` sweeps how many requests each
connection keeps outstanding (pipeline depth).

## Completion topologies

The same echo workload is measured under three completion-delivery topologies:

- **`echo`** (arm-park, default) — per-connection interrupt-armed CQs serviced by a
  shared multi-thread tokio runtime; work-steals across all vCPUs and parks when
  idle. Best deep-pipeline throughput and CPU/op.
- **`echo-busy`** (busy-poll) — `--threads N` pins N cores, each spinning one shared
  CQ at 100 % and demuxing completions by `qp_num`. Lowest latency in the shallow /
  request-response regime; burns every pinned core even when idle.
- **`echo-park`** (thread-per-core arm-park) — pinned `current_thread` cores with
  per-connection armed CQs that **park** in `epoll_wait` when idle (reactor-per-core,
  no work-stealing). The middle point: pinned locality with 0 % idle CPU.
...
- **Other scenarios**: [gRPC](grpc.md) · [HTTP/1.1](h1.md)
